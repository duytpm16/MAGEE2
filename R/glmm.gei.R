glmm.gei2 <- function(null.obj, interaction, geno.file, outfile, interaction_covariates = NULL, center=T, MAF.range = c(1e-7, 0.5), miss.cutoff = 1, missing.method = "impute2mean", nperbatch=100, ncores = 1){
  ...
  if(ncores > 1) {
    ...
    for(i in 1:nbatch.flush) {
      ...
      tmp.out <- lapply(1:((tmp.p-1) %/% nperbatch + 1), function(j) {
        ...
        IV.U <- (rep(1, ncol(E)) %x% diag(ncol(geno))) * as.vector(crossprod(K,residuals))-t(t(KPG)*BETA.MAIN)
        if(!is.null(interaction_covariates)) {
           npb <- length(tmp2.variant.idx)
           STAT.INT <- numeric(npb)
           indU <- seq(1, (ei+qi) * npb, by = npb)
           idx_UE <- indU[(qi+1):(qi+ei)] - 1
           idx_UEC <- indU[1:qi] - 1
        
           IV.V <- KPK - tcrossprod(KPG,t(t(KPG)*V_i))
           for (n in 1:npb) {
                idx_UE <- idx_UE + 1
                idx_UEC <- idx_UEC + 1
                
                IV.U2 = IV.U[idx_UE, n] - matrix(IV.V[idx_UE, idx_UEC], ei, qi) %*% solve(IV.V[idx_UEC, idx_UEC], IV.U[idx_UEC, n]) 
                IV.V2 = IV.V[idx_UE , idx_UE] - IV.V[idx_UE, idx_UEC] %*% solve(IV.V[idx_UEC, idx_UEC], matrix(IV.V[idx_UEC, idx_UE], qi, ei))
                IV.V2 <- try(solve(IV.V2, silent = TRUE))
                suppressWarnings({if(class(IV.V2) == "try-error") IV.V2 <- MASS::ginv(IV.V2)})
                STAT.INT[n] <- crossprod(IV.U2, crossprod(IV.V2, IV.U2))
           }
           PVAL.INT <- pchisq(STAT.INT, df=ei, lower.tail=FALSE)
           PVAL.JOINT <- ifelse(is.na(PVAL.MAIN), NA, pchisq(BETA.MAIN * U + STAT.INT, df=1+ei, lower.tail=FALSE))
